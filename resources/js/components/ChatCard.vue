...
<template>
  ...
  >
    <h4 class="mb-6 px-7.5 text-xl font-semibold text-black">Chats</h4>

    <div>
      <template v-for="(chat, index) in chatData" :key="index">
        <RouterLink
          to="/"
          class="flex items-center gap-5 py-3 px-7.5 hover:bg-gray-3"
        >
          ...
          <div class="flex flex-1 items-center justify-between">
            <div>
              <h5 class="font-medium text-black">{{ chat.name }}</h5>
              <p>
                <span class="text-sm text-black">{{ chat.text }}</span>
